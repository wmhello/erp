...
//    }
    ...
    {
//        $lists  = Cache::get('category');
//        dd($lists);

        $name = '艺朝艺夕教育科技集团';
        $imgPath = 'https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1529051363064&di=242211377b2f109eb82425fa952357a9&imgtype=0&src=http%3A%2F%2Fpic9.photophoto.cn%2F20081208%2F0036036322741929_b.jpg';
        $flag = Mail::send('mail.test', ['name' => $name, 'imgPath' => $imgPath], function ($message) {
            $to = '13577728948@163.com';//接收邮件地址
            $message->to($to)->subject('测试邮件');

            //在邮件中上传附件
            // $message->attach(__FILE__);
            // $message->attach($attachment,['as'=>"=?UTF-8?B?".base64_encode('angelo')."?=.doc"]);
        });
        if (!$flag) {
            echo '发送邮件成功，请查收！';
        } else {
